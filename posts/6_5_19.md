...
Lets go through exactly what had to be done to transform my walls to be normal to the surface of the circle. First, its important to remember that a line is being bent into a circle by this math. The center is not being affected and as you travel to the edges, the vertexes are pushed down by up to the radius to create a circle.
...
- Every vertex needs to be moved on the x axis by it's x coordinate times &delta;
...
Its worth noting that there's a whole lot more that needs to happen.

1 - We need the walls to be updated dynamically for the the destructible and magic layers so they are always accurate
2 - We need 4 copies of the walls so if the player is at a corner, every wall can cast shadows
3 - We need the walls to move in unison to appear to be stationary when compared to the player

To achieve 1, I use this [tutorial](https://www.youtube.com/watch?v=fc3nnG2CG8U) for edge detection in a pixel world. When the level starts, I call the algorithm on every layer and create the lighting walls. Whenever the destructible layer is destroyed or magic is applied, I reapply the edge detection algorithm on those pixels and the ones nearby. Then I recreate the walls based on the updated info.

Achieving 2 is simple because with code I can duplicate the initial walls and force them all to be relative to eachother.

Finally, we can attach the remain stationary script I've already created to each copy of the walls so they never appear to move.
